...
class MAc_money_receipts extends CI_Model {
    ...
    {
        $data = array();
        $this->db->select('receipts.*, emps.name as emp_name,users.name as login_emp_name');
        $this->db->from('ac_money_receipts as receipts');
        $this->db->join('emps', 'receipts.emp_id=emps.id', 'left');
        $this->db->join('users', 'receipts.emp_id=users.id', 'left');//Edited By Razib
        $this->db->where('receipts.company_id', $this->session->userdata('user_company'));
        $this->db->order_by('mr_no','DESC');//Edited By Razib
        $q = $this->db->get();
        if ($q->num_rows() > 0)
        {
            foreach ($q->result_array() as $row)
    ...
    public function create()
    {
        $data = array(
            ...
            'emp_id' => $this->session->userdata('user_id'),//Edited By Razib 
