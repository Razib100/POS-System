...
class Report extends CI_Controller {
    ...
    public function ledger()
    {
        if ($this->input->post())
        {
            ...
            if ($this->input->post('report_type') == 'pdf')
            {
                $html = $this->load->view('admin/reports/accounts/ledger_details', $data, TRUE);
                $pdfFile = "Ledger.pdf";
                $this->load->library('m_pdf');
                $pdf = $this->m_pdf->load();
                $pdf->AddPage('L', '', '', '', '', '', '', '', '', '', '');
    ...
    public function Item(){
        $data['title'] = 'POS System';
        $data['menu'] = 'report';
        $data['content'] = 'admin/reports/inventory/itemStock';
        $data['items'] = $this->MItems->get_all();
        $data['suppliers'] = $this->MSuppliers->get_all();
        $data['privileges'] = $this->privileges;
        $this->load->view('admin/template', $data);
    ...
    public function ItemStockDetails(){
        ...
        if ($this->input->post())
        {
            ...
            if ($this->input->post('report_type') == 'pdf')
            {
                $html = $this->load->view('admin/reports/inventory/itemDetailsPdf', $data, TRUE);
                $pdfFile = "itemQuantity.pdf";
                $this->load->library('m_pdf');
                $pdf = $this->m_pdf->load();
                $pdf->AddPage('L', '', '', '', '', '', '', '', '', '', '');
                $pdf->use_kwt = TRUE;
            ...
            {
                $this->load->view('admin/reports/inventory/itemStockDetails', $data);
            }
        }
        else
        {
