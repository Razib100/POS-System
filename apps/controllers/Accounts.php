...
class Accounts extends CI_Controller {
    ...
    {
        $config = array();
        $config["base_url"] = base_url() . "accounts/journal-list";
        $config["total_rows"] = $this->MAc_journal_master->record_count();
        $config["per_page"] = 10;
        ...
        $data['journals'] = $this->MAc_journal_master->get_all($config["per_page"], $page);
        $data['privileges'] = $this->privileges;
    ...
    public function mr_save($id = NULL)
    {
        ...
        else
        {
            ...
            $data['o_emps'] = $this->MEmps->get_all();
            $data['emps'] = $this->session->user_name;//Edited By Razib
            $data['privileges'] = $this->privileges;
            $this->load->view('admin/template', $data);
        }
    }
