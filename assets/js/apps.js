$(document).ready(function() {
	...
	$(document).on('click', '#sales_complete', function(event) {
		...
		$.ajax({
			...
			data: {sales_no: sales_no, paid_amount: paid_amount,discount: discount, sales_date: sales_date, customer_id: customer_id, emp_id: emp_id, notes: notes},
			success: function(msg) {
	...
	$('#data_table_desc').DataTable({
		responsive: true,
		"order": [[ 0, "desc" ]],
		//edited by Razib hasan
		// "scrollY": 200,
        "scrollX": true,
	});
// Acoount Journal Datatable Edited By Razib
	$('#account_journal_datatable').DataTable({
		"bInfo" : false,
		"bPaginate": false,
		responsive: true,
		"order": [[ 0, "desc" ]],
	...
	$('#item_id.change-price').on('select2:select', function (e) {
		...
		$.ajax({
			type: "POST",
			url: "inventory/get_item_quantity",
			data: {id: select_data.id},
			success: function(msg) {
				$("#ab_quantity").val(msg);
			}
